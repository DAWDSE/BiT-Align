...
class TestData(data.Dataset):
            ...
            for obj_file in obj_files:
                ...
                for img in images:
                    ...
                    if os.path.exists(mask_path):
                        ...
                        depth_img_path = img_path.replace("AGD20K", "AGD20K-Depth")
                        filename, ext = img_path.rsplit('.', 1)
                        depth_img_path = filename + '_depth.' + 'png'
    ...
    def __getitem__(self, item):
        ...
        filename, ext = image_path.rsplit('.', 1)
        depth_img_path = filename + '_depth.' + 'png'   
        depth_img_path = depth_img_path.replace("AGD20K", "AGD20K-Depth")     
        depth_image = self.load_depth_img(depth_img_path)

        label = self.aff_list.index(aff_name)
        names = image_path.split("/")
        mask_path = os.path.join(self.mask_root, names[-3], names[-2], names[-1][:-3] + "png")

        return image, depth_image, label, mask_path, ego_label_name
