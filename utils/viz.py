...
def viz_pred_train(args, ego, exo, masks, aff_list, aff_label, epoch, step):
    ...
    num_head = masks['ego_samss'].size(1)
    sam_egos = masks['ego_samss']

    head_idxs = masks['head_idxs']
    attn_cosine_sim = masks['attn_cosine_sim']
    
    sam_images = []
    for head in range(num_head):
        sam_ego = sam_egos[0, head]
        sam_ego = np.array(sam_ego.squeeze().data.cpu())
        sam_ego = normalize_map(sam_ego, args.crop_size)
        sam_ego = Image.fromarray(sam_ego)
        overlay = overlay_mask(ego_0, sam_ego, alpha=0.1)
        title = f"Head {head} \n {attn_cosine_sim[0, head]}"
        sam_images.append((overlay, title))
    ...
    fig, ax = plt.subplots(5, num_head, figsize=(14, 14))
    for axi in ax.ravel():
        axi.set_axis_off()
    for k in range(num_exo):
        ax[0, k].imshow(eval('exo_aff' + str(k)))
    ...
    ax[3, 0].set_title(aff_str)
    ax[3, 1].imshow(ego_sam)
    ax[3, 1].set_title('Saliency' + f"head_idxs {head_idxs[0]}")
    for k in range(num_head):
        ax[4, k].imshow(sam_images[k][0])
        ax[4, k].set_title(sam_images[k][1])
